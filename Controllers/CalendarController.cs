...
public class CalendarController : ControllerBase
{
	...
	[HttpGet]
	[Route("dates")]
	public IActionResult GetCalendarDatesForClient([FromQuery] string clientId)
	{
		var user = _userService.GetById(long.Parse(clientId));

		if (user.Role == UserRole.Client)
		{
			var reservations = _flightReservationService.GetCalendarDatesForClient(user.Id);
			return Ok(reservations);
		}
		else
		{
			var reservations = _flightReservationService.GetCalendarDatesForInstructor(user.Id);
			return Ok(reservations);
		}
	}
}